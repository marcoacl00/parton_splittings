...
def main(file_name):
    ...
    grid_k2 = np.linspace(-eps, Lk - eps, Nk2)
    ...
    Fmed_theo = Theta**2 /2 * np.real(f_vals_theo)

    plt.plot(Theta, Fmed_theo, label=r"Large $N_c$ fac.", linestyle='--', color = 'blue')
    ...
    np.save(f"saved_results/F_in-in_arrays_E={E/fm:.1f}_z={z:.2f}_qhat={qhat/fm**2:.1f}_L={L:.1f}_NcMode={NcMode}.npy",
            np.array([Theta, Fmed_theo, F_med_sim, NcMode], dtype=object))
