...
def faber_params_new_momentum(sys):
    qF = sys.qhat
    dk1 = sys.dk1
    dk2 = sys.dk2
    dl1 = sys.dl1
    ...
    lam_re_min = -lam_re_max
    
    #Maximum imag eigenvalue
    d_d2_eig = - 4 * (1/dk1**2 + 1/dk2**2) - 2 * (1/dl1**2 + 1/dl2**2)
...
def faber_expand_new_momentum(sys, ht, gamma0, gamma1, one_lamb, coeff_array=None):
    """Computes e^(-iHdt)f using the Faber expansion for the new momentum method. 
    Takes a 4D vector F, the number of polynomials 
    Np and the potential field V"""
    
    fH_0 = 1.0 * sys.Fsol

    fH_1 = apply_hamil_new_mom_par(sys, fH_0) * one_lamb
    fH_1 -= gamma0 * fH_0

    fH_2 = apply_hamil_new_mom_par(sys, fH_1) * one_lamb
    fH_2 += -gamma0 * fH_1 - 2 * gamma1*fH_0
